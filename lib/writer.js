...
class ParquetEnvelopeWriter {
  ...
  writeIndex(_rowGroups) {
    ...
    return Promise.all(this.schema.fieldList.map((c, i) => {
      return Promise.all(rowGroups.map(group => {
        ...
        if (column.meta_data.columnIndex) {
          ...
          return this.writeSection(columnBody);
        }
        ...
        if (column.meta_data.offsetIndex) {
          ...
          return this.writeSection(offsetBody);
        }
      }).filter(x => x));
    }));
  }
